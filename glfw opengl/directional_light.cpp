...
int main()
{
    ...
    // ------------------------------------
    Shader lightingShader("vertex7.vert", "fragment7.frag");
    Shader lightCubeShader("vertex7.vert", "fragment_light_source2.frag");

    lightingShader.use();
    // lightingShader.setVec3("light.position", lightPos);
    glm::vec3 lightDir(-1.f, -.5f, 0.f);
    lightDir *= .1f;
    ...
    // load the textures
    // const char* texture_paths[] = { "assets/container.png", "assets/container_specular.png" };
    const char* texture_paths[] = { "assets/container.png", "assets/container_specular.png", "assets/matrix.jpg" };
    const int n_tex = 3;
    unsigned int textures[n_tex];
    ...
    for (int i = 0; i < n_tex; i++) {
        ...
        if (data)
        {
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, i != 2 ? GL_RGBA : GL_RGB, GL_UNSIGNED_BYTE, data);
    ...
    srand(time(0));
    const int n_cubes = 150;
    float radius = 15.f;
    glm::vec3 positions[n_cubes];
    for (int i = 0; i < n_cubes; i++) {
        glm::vec3 pos(randf() - .5f, randf() - .5f, randf() - .5f);
        positions[i] = glm::vec3(glm::normalize(pos) * (randf() * radius));
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // test, draw the light direction
        glm::vec3 lightDirCpy(lightDir);
        
        int n = 50;
        glm::vec3 offset = lightDir * (float)(n / 2);
        for (int i = 0; i < n; i++) {
            ...
            model = glm::mat4(1.0f);
            model = glm::translate(model, lightDirCpy - offset);
