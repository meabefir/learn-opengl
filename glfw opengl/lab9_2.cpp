...
#define N_V (N * 2 + 4)
    ...
    unsigned int upperBaseOffset = N + 2;
    ...
    for (int i = 0; i <= N; i++) {
        ...
        vertices[i + 1] = glm::vec3(glm::cos(currentDelta), -1.f, glm::sin(currentDelta));
        vertices[i + 1 + upperBaseOffset] = glm::vec3(glm::cos(currentDelta), 1.f, glm::sin(currentDelta));
    }

    glm::vec3 colors[N_V];
    for (int i = 0; i < N_V; i++) {
        colors[i] = glm::vec3(0.f, 1.f, 0.f);
    }
    glm::vec3 colors2[N_V];
    for (int i = 0; i < N_V; i++) {
        colors2[i] = glm::vec3(0.f, 0.f, 1.f);
    }
    ...
    unsigned int vbo, vbo_color, vbo_color2, vao, vao2, ebo;
    glGenBuffers(1, &vbo);
